...
- 倒计时槽位最多 3 项，支持每周某天、每月某日、指定日期（可设每年重复），放不下自动折行
- 图片按自己的宽高比完整显示，绝不裁切：竖图放在文字旁边，横图放在文字上方，尺寸由卡片高度（或宽度）决定；边缘可淡出
- 排法由卡片形状决定：宽 > 高 时图在左、字在右；高 ≥ 宽 时图在上（顶满上左右三边）、字在下
- 文字撑不满时可自动把卡片加宽（横版，最多 2.5 倍）或加高（竖版，最多 1.6 倍）
- 外观页可按屏幕大小一键套用横版 / 竖版尺寸
